...
class PageList_PPWalletTran extends Component {
  constructor(props) {
    ...
    this.API_URL = "PPWalletTran/0/token";
  ...
  refund = async (row) => {
    console.log(row);
    if (row.txn_merchant_code != null) {
      let vformData = new FormData();
      vformData.append("merchant_code", row.txn_merchant_code);
      vformData.append("RefId", row.TransactionCode);
      ...
      if (responseData.response_code === 1) {
        // Store temporary data and show OTP modal
        this.setState({
          otpModal: true,
          tempStateResp: responseData.data.stateresp,
          tempMerchantCode: row.txn_merchant_code,
          tempRefId: row.TransactionCode
        });
      }
    }
  };
  ...
  handleOtpSubmit = async () => {
    try {
      let vformData = new FormData();
      vformData.append("merchant_code", this.state.tempMerchantCode);
      vformData.append("RefId", this.state.tempRefId);
      vformData.append('otp', this.state.otp);
      vformData.append('stateresp', this.state.tempStateResp);
      ...
      if (responseData.response_code === 3) {
        ...
        this.setState({ 
          success_msg: true,
          title: 'OTP Mismatch! Please try again.'
        });
      } else if (responseData.response_code === 1) {
        // Refund successful
        this.setState({ 
          success_msg: true,
      ...
      // Close the OTP modal and clear the OTP input
      this.setState({ otpModal: false, otp: '' });

    } catch (error) {
      console.error("Error submitting OTP:", error);
      this.setState({ 
        success_msg: true,
        title: 'Error processing refund. Please try again.'
      });
    }
  };
  ...
  render() {
...

    {
      ...
      formatter: (cellContent, row) => {
        if (row.IsRefundDone === 1) {
          // Show refund button when refund is available
          return (
            <button
              type="button"
              className="btn btn-warning btn-xs"
              onClick={() => this.refund(row)}
            >
              Process Refund
            </button>
          );
        } else if (row.IsRefundDone === 2) {
          // Show disabled button when refund is already done
          return (
            <button
              type="button"
        ...
        } else if (row.IsRefundDone === 0) {
          // Show success message for completed transactions
          return (
        ...
        <React.Fragment>
          ...
          <div className="page-content">
            ...
            <Modal
              ...
              toggle={() => this.setState({ otpModal: false })}
              centered
            >
              <div className="modal-header">
                ...
                <button
                  type="button"
                  className="btn-close"
                  onClick={() => this.setState({ otpModal: false })}
                  aria-label="Close"
                ></button>
              </div>
              <div className="modal-body">
