...
class BusesList extends Component {
  ...
  viewseats = async(busId,busName,busType, bus) => {
    this.setState({ isSeatsLoading: true, selectedBusId: busId, showSeats: !this.state.showSeats, busdata: bus });
    const formData = new FormData();
          ...
          return (
            ...
            <div
              ...
              style={{
                ...
                fontWeight: 600,
                fontSize: isMobile ? (isSleeper ? 14 : 12) : (isSleeper ? 18 : 16),
            ...
            (buses && buses[0] && buses[0].buses ? buses[0].buses : []).map(bus => {
              return (
                <React.Fragment key={bus.id}>
                  <div style={{ background: '#fff', borderRadius: 24, boxShadow: '0 2px 8px #0001', padding: isMobile ? 16 : 32, display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'space-between', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 12 : 0 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? 12 : 24, width: isMobile ? '100%' : 'auto', flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
                        ...
                        <div style={{ color: '#888', fontSize: isMobile ? 14 : 16 }}>
                          {(() => {
                            const dep = bus.timeD;
                            const arr = bus.timeA;
                            if (dep && arr) {
                      ...
                      <div style={{ textAlign: isMobile ? 'left' : 'right', width: isMobile ? '100%' : 'auto' }}>
                        <div style={{ fontWeight: 700, fontSize: isMobile ? 20 : 24, color: '#222' }}>₹{bus.fares[0].base} <span style={{ color: '#888', fontWeight: 400, textDecoration: bus.fares[0].base+100 ? 'line-through' : 'none', fontSize: isMobile ? 16 : 18, marginLeft: 8 }}>{bus.fares[0].base+100 ? `₹${bus.fares[0].base+100}` : ''}</span></div>
                        <div style={{ color: '#888', fontSize: 14 }}>Onwards</div>
                      </div>
                      <button onClick={() => this.viewseats(bus.id,bus.name,bus.type,bus)} style={{ background: '#d44', color: '#fff', border: 'none', borderRadius: 32, padding: isMobile ? '12px 20px' : '16px 32px', fontWeight: 700, fontSize: isMobile ? 18 : 20, marginLeft: isMobile ? 0 : 24, cursor: 'pointer', width: isMobile ? '100%' : 'auto' }}>View seats</button>
                    </div>
                  </div>
                  {showSeats && selectedBusId === bus.id && (
                    <div style={{ background: '#fff', borderRadius: 16, padding: isMobile ? 16 : 24, margin: '24px 0', boxShadow: '0 2px 8px #0001', width: isMobile ? '100%' : undefined }}>
                      {this.state.isSeatsLoading ? (
                        <div style={{ textAlign: 'center', fontWeight: 600 }}>Loading seats...</div>
                      ) : (
                        <>
                          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 32, width: '100%' }}>
                            <div>
                            ...
                            <div>
                              ...
                              {this.renderSeatGrid(seatData, 1, this.getGridBounds(seatData, 0).maxX * (isMobile ? 32 : 48) + 32)}
                            </div>
                          </div>
